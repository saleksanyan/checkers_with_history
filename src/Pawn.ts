...
class Pawn extends Figure{
    ...
    allDestinations(position: Position, board: Board,
         afterEating: boolean,  allDestinations: Position[],
        moves: Move[] 
    ): Position[] {
        ...
        for (let reachableRow = 0; reachableRow < 
            this.reachablePositionsWithoutEating.length; reachableRow++) {
            for (let reachableColumn = 0; reachableColumn < 
                this.reachablePositionsWithoutEating.length; 
                reachableColumn++) {
                ...
                if (Validations.isValidPlace(eatableFigureRow, eatableFigureColumn)) {
                    if (Validations.placeIsEmpty
                        ...
                        moves.push(new Move(position, new Position(Position.
                            getPositionUsingBoardPlaces(eatableFigureRow, eatableFigureColumn)))); 
                    } else if (!Validations.placeIsEmpty
                        (eatableFigureRow, eatableFigureColumn, board) && 
                        ...
                        if (figure instanceof Figure && 
                            ...
                            let nextPos = HelpingFunctions.addingPositionToArray(figuresNewRow, 
                                figuresNewColumn, allDestinations);
                            moves.push(new Move(position, new Position(
                                Position.getPositionUsingBoardPlaces
                                (figuresNewRow, figuresNewColumn))));
    
                            this.allDestinations
                            (nextPos, board, true, allDestinations, moves);
                        }
                    }
                }
            }
